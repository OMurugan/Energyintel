...
def register_callbacks(dash_app, server):
    ...
    def update_asia_table(slider_range, selected_unit, selected_sector, selected_countries, table_granularity, date_list_iso):
            ...
            if current_vals_at_depth:
                ...
                for i, v in enumerate(current_vals_at_depth):
                    if v == curr_val:
                        ...
                        # Get parent path from first item in this group
                        # (index of start of group is i - count)
                        sample_col_idx = i - count
                        path_parts = cols_tuples[sample_col_idx][:depth+1] # Include current level
        ...
        return html.Div(
            html.Table(
                ...
                id='asia-demand-table',
                style={'borderCollapse': 'collapse', 'width': '100%', 'fontFamily': 'Arial', 'fontSize': '12px', 'cursor': 'default'}
            ),
            style={'overflowX': 'auto', 'maxWidth': '100%', 'width': '100%', 'maxHeight': '650px'}
        )
    ...
    dash_app.clientside_callback(
        ...
        function(table_children) {
            try {
                const tableId = 'asia-demand-table';
                const baseStyleId = 'asia-demand-table-base-css';
                const dynamicStyleId = 'asia-demand-table-dynamic-highlight-css';
                ...
                const setupTable = () => {
                    ...
                    tableEl.addEventListener('click', function(e) {
                        ...
                        // Ignore clicks on Total if desired, or handle them. Allowing for now.
                        if (colId === 'Country' && !rowIndex) return;
                        
                        const isHeader = !!header;
                        
                        // Toggle logic (using simplistic key for now)
                        ...
                        if (isHeader) {
                            ...
                            // If it's a parent header (no specific logic needed if IDs are shared with children, 
                            // BUT our IDs are specific paths "Time | 2023 | Q1".
                            // If I click "2023", ID is "Time | 2023".
                            // Children will have IDs starting with "Time | 2023 |".
                            
                            // Discovery: Find all columns that start with this ID
                            ...
                            // Filter IDs that start with the clicked header ID (prefix match for hierarchical columns)
                        ...
                        if (targetColumnIds.length > 0) {
                            targetColumnIds.forEach(id => {
                                // Escape CSS selector special chars if any (basic assumption: safe chars)
                                dynamicStyles += `
                ...
                console.error('Asia Table highlighting error:', e); 
                return ""; 
            }
        }
        """,
